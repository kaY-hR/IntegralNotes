...
新規作成する解析 script は `scripts/ai_blocks/` などの visible path に置く。`.packages` / `.integral-sdk` / `.store` / `.inline-action` / `.codex` / `.claude` など hidden/system folder 配下には生成しない。package 由来 script を参考にする場合も、変更版は visible path に copy / adapt してから挿入する。
...
既存 block が `.packages/...` の package import を指す場合、package 側を直接変更しない。挙動を変える必要があるときは visible path に変更版 script を作り、その visible script の block を挿入する。
...
- `scriptPath` が `.packages` など `.` 始まりの hidden/system path component を含む場合は validation error とし、visible path への保存を促す
