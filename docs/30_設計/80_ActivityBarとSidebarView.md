...
必要なら Activity Bar から main workspace tab を開く built-in command item も置けるようにする。
...
最も左の縦アイコン列。  
ここでは `sidebar view` を切り替える item と、main workspace の tab を開く item を並べてよい。
...
### built-in command items

- provenance pane
- 将来的な AI Chat など、main workspace 側に tab を開く item
...
provenance pane は sidebar view ではなく、Activity Bar から開く main workspace tab とする。
