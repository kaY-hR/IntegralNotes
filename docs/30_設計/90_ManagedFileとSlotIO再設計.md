...
- 単一 file 入力は managed data ID で扱う
- output は実行前 path、実行後 managed data ID で扱う
- `1 slot = 1 managed data reference` を MVP の基本契約にする
...
### 4. Markdown source と block I/O の責務を分ける

- `[label](path)` / `![](path)` はそのまま使う
- 通常 Markdown link / image は path 記法のままにする
- `itg-notes` block の input は保存上 ID ベースで持つ
- `itg-notes` block の output は実行前 path、実行後 ID で持つ
...
datatype は解析間の I/O 互換性を表す任意の string とする。
MVP では registry や別 ID は持たず、managed file / dataset metadata と slot 定義に直接保存する。
...
- input は `target_id` を正とする
- 実行前 output は `planned_path` を正とする
- 実行後 output は `target_id` を正とする
...
- `extension`
  - output で生成する suffix
- `datatype`
  - slot が要求または生成する semantic data type

`.idts` は bundle の物理表現を表す拡張子であり、datatype そのものではない。
