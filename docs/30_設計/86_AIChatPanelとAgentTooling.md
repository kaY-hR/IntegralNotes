...
- runtime は `Vercel AI Gateway 固定` ではなく、
  ...
  の順で選ぶ
- 汎用探索は `bash-tool` を使う
- agent runtime の skills は workspace の `.codex/skills` / `Notes/.codex/skills`、IntegralNotes global skill (`%LocalAppData%/IntegralNotes/global/skills`)、package exported skill (`%LocalAppData%/IntegralNotes/packages/*/exports.skills`) を読み、`experimental_createSkillTool()` で渡す
- Codex 本体の global skill (`%UserProfile%/.codex/skills`) は IntegralNotes の LLM には渡さない
- workspace 初期化時は app 同梱 workspace template を展開し、その一部として root `.codex/skills` / `.claude/skills` を配置する
...
Main Process
  ...
  AiAgentService
    ...
    -> bash-tool runtime
    -> skill tool (workspace / IntegralNotes app global skills)
...
- workspace `.codex/skills` を最優先で読む
- workspace `Notes/.codex/skills`、IntegralNotes global skill (`%LocalAppData%/IntegralNotes/global/skills`)、package exported skill (`%LocalAppData%/IntegralNotes/packages/*/exports.skills`) も読む
- Codex 本体の global skill (`%UserProfile%/.codex/skills`) は読まない
- `.codex/skills` / `.claude/skills` は workspace template の一部として配布し、workspace root へ展開する
- 開発時の template source は source tree の `Notes/`、packaged app の template source は `process.resourcesPath/workspace-template/`
...
- `png`, `jpg`, `webp`, `svg` など主要画像を読み、model には既定で長辺 512px 程度・120KB 目安の JPEG thumbnail として返す
- 必要に応じて `crop: { x, y, width, height }` を source image pixel で指定し、特定領域だけを切り出して model に渡せる
...
- model には既定で screenshot の長辺 512px 程度・120KB 目安の JPEG thumbnail を返す
- 必要に応じて `crop: { x, y, width, height }` を rendered screenshot pixel で指定し、特定領域だけを切り出して model に渡せる
