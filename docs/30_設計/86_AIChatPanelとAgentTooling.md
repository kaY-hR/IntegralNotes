...
- runtime は `Vercel AI Gateway 固定` ではなく、
  ...
  の順で選ぶ
- 汎用探索は `bash-tool` を使う
- agent runtime の skills は workspace の `.codex/skills` / `Notes/.codex/skills` と IntegralNotes app global skill (`%LocalAppData%/IntegralNotes/skills`) を読み、`experimental_createSkillTool()` で渡す
...
- workspace `Notes/.codex/skills` と IntegralNotes app global skill (`%LocalAppData%/IntegralNotes/skills`) も読む
