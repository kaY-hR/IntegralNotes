...
description: "Use when implementing or revising an IntegralNotes Python analysis block based on `from integral import integral_block`, including `run: relative/path.py:function` note blocks, dataset input and output handling, and demo scripts under `Notes/`."
---
...
- Treat every input and output slot as a dataset directory path at runtime.
...
- Write stable UTF-8 output files under the assigned output slot directory.
...
- Do not invent decorator features that do not exist. The current SDK only accepts `display_name`, `description`, `inputs`, and `outputs`.
...
- output files are written under the assigned output slot directory
